...
// The length of the shorter timer
const long SHORT_TIMER_MINS = (7.5 * 60) + MINS_TO_FALL_ASLEEP; 
// The length of the longer timer
...
#define shortTimer 3 // The LED that indicates the choice of the shorter timer
#define longTimer 4 // The LED that indicates the choice of the longer timer  
...
void loop(){
  ...
  boolean choseShortTimer = waitToStartTimer();
  sleepWithTimer(choseShortTimer);
  soundAlarm();
  hibernate();
}
...
boolean waitToStartTimer(){
  boolean choseShortTimer = true;
  int thisQuarter = 0; 

  // Light the shortTimer LED by default
  ...
  while(true){
    ...
    if(buttonPressed(startTimer) == true){
      ...
      return choseShortTimer;
    }
    
    // If the pickTimer button has been pressed
    else if(buttonPressed(pickTimer) == true){
      // Toggle between the shortTimer and longTimer LEDs
      if(choseShortTimer == true){
        digitalWrite(shortTimer, LOW);
        digitalWrite(longTimer, HIGH);
        choseShortTimer = false;
      }else{
        digitalWrite(longTimer, LOW);
        digitalWrite(shortTimer, HIGH);
        choseShortTimer = true;
      }
    }
...
void sleepWithTimer(boolean choseShortTimer){  
  long minsToSleep;
